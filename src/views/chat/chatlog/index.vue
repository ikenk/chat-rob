...
const ensureName = async ()=>{
  ...
  if(rename.value===''){ isEdited.value = false; return}
  const res = await renameChatLog(route.params.params,{name:rename.value})
  prop.chatHistory?.forEach(
    (item)=>{
      item.chatlogs.forEach(
        (item)=>{
  ...
  <div class="container">
    <el-scrollbar class="scroller">
      <div class="chatlogbox" v-for="logOnDate in chatHistory">
        ...
        <div class="content" :class="{'active': datalog.chatlogID === indexActive }" v-for="datalog in logOnDate.chatlogs" :key="datalog.chatlogID" @click="chooseDialog(datalog.chatlogID)">
          <icon-svg icon-class="shenqingapplications" class="text"></icon-svg>
          <el-input v-model="rename" class="edit-input" @click.stop v-if="isEdited && datalog.chatlogID === indexActive" placeholder="Please input" clearable />
          <el-text class="el-text" truncated v-else>{{ datalog.chatTitle }}</el-text>
          <icon-svg icon-class="bianji" v-if="datalog.chatlogID === indexActive && !isEdited" class="edit" @click.stop="editChatName"></icon-svg>
          <icon-svg icon-class="select" v-if="datalog.chatlogID === indexActive && isEdited" class="edit" @click.stop="ensureName"></icon-svg>
          <icon-svg icon-class="close" v-if="datalog.chatlogID === indexActive && isEdited" class="edit" @click.stop="cancelRename"></icon-svg>
          <icon-svg icon-class="shanchu" v-if="datalog.chatlogID === indexActive && !isEdited"  class="delete" @click.stop="delChat"></icon-svg>
        </div>
      </div>
    </el-scrollbar>
    <el-dialog
